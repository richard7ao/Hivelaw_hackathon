...
<html><head><meta charset="utf-8"><title>${reportData.title} — Steelman Report</title>
<style>
body{font-family:Georgia,serif;max-width:680px;margin:40px auto;padding:0 24px;color:#333;line-height:1.7;font-size:14px}
h1{font-size:20px;border-bottom:2px solid #8b1a1a;padding-bottom:8px;margin-bottom:4px}
h2{font-size:15px;color:#8b1a1a;margin-top:28px;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.05em}
.subtitle{color:#666;font-size:13px;margin-bottom:24px}
.support{background:rgba(76,150,80,0.12);padding:1px 4px;border-radius:2px;color:#2d6a30}
.flag{background:rgba(200,50,50,0.12);padding:1px 4px;border-radius:2px;color:#8b2020}
.verdict{display:inline-block;border:2px solid #b8860b;background:rgba(184,134,11,0.08);padding:4px 12px;border-radius:6px;font-weight:bold;color:#8b6914;margin:8px 0}
.citation{font-size:12px;color:#555;border-left:3px solid #8b1a1a;padding-left:12px;margin:6px 0}
.letter{white-space:pre-wrap;font-size:14px;line-height:1.7;margin:16px 0}
...
<div class="verdict">PROSPECTS: ${caseProspects.toUpperCase()}</div>
<h2>Case Assessment</h2>
${reportData.paragraphs.map((p) => {
      let text = p.text;
      p.highlights.forEach((h) => { text = text.replace(h.text, `<span class="${h.type === "support" ? "support" : "flag"}">${h.text}</span>`); });
      return `<p>${text}</p>`;
    }).join("")}
<h2>Arguments For</h2>
...
<div class="letter">${MOCK_SUBMISSION_PREVIEW}</div>
...
<div class="footer">Generated by Steelman &mdash; Case Assessment AI &bull; ${new Date().toLocaleDateString("en-GB")}</div>
</body></html>`;
